#!/usr/bin/env bash
# Build gRPC (submodule third_party/grpc, with its bundled protobuf/abseil/boringssl/re2/c-ares/zlib) as
# static libraries with the Unreal Engine Linux toolchain (clang + libc++ + sysroot), and bundle them into
# ONE archive that the ManipleGrpc third-party module links:
#   ue/Plugins/ManipleInference/Source/ThirdParty/ManipleGrpc/lib/Linux/libmaniple_grpc.a
#   ue/Plugins/ManipleInference/Source/ThirdParty/ManipleGrpc/include/{grpc,grpcpp,google,absl}
#   ue/Plugins/ManipleInference/Source/ThirdParty/ManipleGrpc/tools/{protoc,grpc_cpp_plugin}   (host tools, for gen_triton_protos.sh)
#
...
[ -f "$SRC/CMakeLists.txt" ] || { echo "submodule missing: git submodule update --init --recursive third_party/grpc"; exit 1; }
[ -x "$TC/bin/clang++" ] || { echo "UE toolchain not found under $UE_ROOT"; exit 1; }
echo "toolchain: $TC"
...
cmake -G Ninja -S "$SRC" -B "$BUILD" \
  ...
  -DCMAKE_C_FLAGS="$FLAGS" -DCMAKE_CXX_FLAGS="$FLAGS -stdlib=libc++" \
  ...
  -DgRPC_BUILD_TESTS=OFF -DgRPC_BUILD_CODEGEN=ON -DgRPC_BUILD_GRPC_CPP_PLUGIN=ON \
  -DgRPC_BUILD_CSHARP_EXT=OFF -DgRPC_BUILD_GRPC_CSHARP_PLUGIN=OFF -DgRPC_BUILD_GRPC_NODE_PLUGIN=OFF \
  -DgRPC_BUILD_GRPC_OBJECTIVE_C_PLUGIN=OFF -DgRPC_BUILD_GRPC_PHP_PLUGIN=OFF -DgRPC_BUILD_GRPC_PYTHON_PLUGIN=OFF \
  -DgRPC_BUILD_GRPC_RUBY_PLUGIN=OFF \
  -DgRPC_ABSL_PROVIDER=module -DgRPC_PROTOBUF_PROVIDER=module -DgRPC_SSL_PROVIDER=module \
  -DgRPC_ZLIB_PROVIDER=module -DgRPC_CARES_PROVIDER=module -DgRPC_RE2_PROVIDER=module \
  -DgRPC_INSTALL=OFF -Dprotobuf_INSTALL=OFF -Dprotobuf_BUILD_TESTS=OFF -Dutf8_range_ENABLE_INSTALL=OFF -DABSL_ENABLE_INSTALL=OFF
...
{ echo "CREATE $OUT/lib/Linux/libmaniple_grpc.a"; for l in $LIBS; do echo "ADDLIB $l"; done; echo "SAVE"; echo "END"; } | "$TC/bin/llvm-ar" -M
