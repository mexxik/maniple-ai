...
FManipleBatchInferer::FManipleBatchInferer(TSharedPtr<FManipleTritonClient> InClient, const FString& InModel,
	const FString& InInputName, int32 InInDim, const FString& InOutputName, int32 InOutDim)
	: Client(InClient), Model(InModel), InputName(InInputName), OutputName(InOutputName), InDim(InInDim), OutDim(InOutDim)
{
}
...
bool FManipleBatchInferer::Flush(FBatchCallback OnBatchDone)
{
	...
	if (N == 0 || !Client.IsValid()) return false;
	...
	Client->Infer(Model, { MoveTemp(Input) }, FManipleInferComplete::CreateLambda([Pending, Out, Dim, N, OnBatchDone](const FManipleInferResult& R)
	{
		const FManipleTensor* T = R.bSuccess ? R.FindOutput(Out) : nullptr;
		const bool bOk = T && T->AsFloats().Num() == N * Dim;
		if (!bOk && R.bSuccess)
		{
			UE_LOG(LogManipleInference, Warning, TEXT("batch: output '%s' has %d floats, expected %d x %d"), *Out, T ? T->AsFloats().Num() : 0, N, Dim);
		}
		for (int32 i = 0; i < N; ++i)
		{
			(*Pending)[i](bOk, bOk ? T->AsFloats().Slice(i * Dim, Dim) : TConstArrayView<float>());
		}
		if (OnBatchDone) OnBatchDone(R, N);
	}), { Out });
	return true;
}
