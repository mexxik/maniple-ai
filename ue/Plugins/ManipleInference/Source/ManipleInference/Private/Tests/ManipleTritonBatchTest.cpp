...
bool FManipleTritonBatchTest::RunTest(const FString& Parameters)
{
	...
	FString Json;
	if (!TestTrue(TEXT("reference json loads"),
			Plugin.IsValid() &&
				FFileHelper::LoadFileToString(Json, *FPaths::Combine(Plugin->GetBaseDir(), TEXT("Resources/policy_ref.json")))))
		return false;
	TSharedPtr<FJsonObject> Ref;
	if (!TestTrue(TEXT("reference json parses"), FJsonSerializer::Deserialize(TJsonReaderFactory<>::Create(Json), Ref) && Ref.IsValid()))
		return false;
	const FString Model = Ref->GetStringField(TEXT("model")), InName = Ref->GetStringField(TEXT("input")),
				  OutName = Ref->GetStringField(TEXT("output"));
	const int32 ObsDim = Ref->GetIntegerField(TEXT("obs_dim")), ActDim = Ref->GetIntegerField(TEXT("act_dim")),
				RefBatch = Ref->GetIntegerField(TEXT("batch"));
	TArray<float> RefObs, RefAct;
	for (const auto& V : Ref->GetArrayField(TEXT("obs")))
		RefObs.Add((float)V->AsNumber());
	for (const auto& V : Ref->GetArrayField(TEXT("action")))
		RefAct.Add((float)V->AsNumber());
	...
	TArray<TArray<float>> RowOut;
	RowOut.SetNum(RefBatch);
	int32 Done = 0, Failed = 0;
	for (int32 i = 0; i < RefBatch; ++i)
	{
		Batch.Submit(TConstArrayView<float>(RefObs).Slice(i * ObsDim, ObsDim),
			[&, i](bool bOk, TConstArrayView<float> Out)
			{
				++Done;
				if (!bOk)
					++Failed;
				else
					RowOut[i] = TArray<float>(Out.GetData(), Out.Num());
			});
	}
	TestEqual(TEXT("pending rows"), Batch.NumPending(), RefBatch);
	bool bBatchDone = false;
	double BatchMs = 0;
	if (!TestTrue(TEXT("flush sends"),
			Batch.Flush(
				[&](const FManipleInferResult& R, int32 Rows)
				{
					bBatchDone = true;
					BatchMs = R.LatencyMs;
				})))
	...
	while (!bBatchDone && FPlatformTime::Seconds() < Deadline)
	{
		Client->PumpCompletions();
		FPlatformProcess::Sleep(0.0005f);
	}
	if (!TestTrue(TEXT("batch completed"), bBatchDone))
		return false;
	...
	for (int32 i = 0; i < RefBatch; ++i)
	{
		if (!TestEqual(FString::Printf(TEXT("row %d size"), i), RowOut[i].Num(), ActDim))
			return false;
		const int64 Shape[2] = {1, ObsDim};
		FManipleInferResult Single =
			Client->InferSync(Model, {FManipleTensor::MakeFloat(InName, Shape, TConstArrayView<float>(RefObs).Slice(i * ObsDim, ObsDim))});
		if (!TestTrue(FString::Printf(TEXT("single row %d ok"), i), Single.bSuccess))
			return false;
	...
	UE_LOG(LogManipleInference, Display,
		TEXT("[batch] %d rows in one request: %.2f ms, max err vs reference %.3g, vs individual requests %.3g"), RefBatch, BatchMs,
		MaxErrRef, MaxErrIndividual);
