...
bool FManipleTritonSmokeTest::RunTest(const FString& Parameters)
{
	...
	// 1) correctness
	{
		...
		if (!TestTrue(FString::Printf(TEXT("infer ok: %s"), *R.Error), R.bSuccess))
			return false;
		const FManipleTensor* Out = R.FindOutput(OutName);
		if (!TestNotNull(TEXT("output present"), Out))
			return false;
		...
		if (!TestEqual(TEXT("output element count"), Act.Num(), RefAct.Num()))
			return false;
		float MaxErr = 0.f;
		for (int32 i = 0; i < Act.Num(); ++i)
			MaxErr = FMath::Max(MaxErr, FMath::Abs(Act[i] - RefAct[i]));
		UE_LOG(LogManipleInference, Display, TEXT("[smoke] %s v%s batch %d: max abs err %.3g, round-trip %.2f ms"), *R.ModelName,
			*R.ModelVersion, RefBatch, MaxErr, R.LatencyMs);
	...
	for (int32 B : {1, 64, 512})
	{
		...
		for (int32 k = 0; k < Iters; ++k)
		{
			...
			if (!TestTrue(FString::Printf(TEXT("batch %d iter %d ok: %s"), B, k, *R.Error), R.bSuccess))
				return false;
			Sum += R.LatencyMs;
			Best = FMath::Min(Best, R.LatencyMs);
		}
		UE_LOG(LogManipleInference, Display, TEXT("[smoke] batch %4d: round-trip best %.2f ms avg %.2f ms (%.1f us/agent)"), B, Best,
			Sum / Iters, Sum / Iters * 1000.0 / B);
	}
	return true;
}
